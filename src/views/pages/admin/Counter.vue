...
<template>
    ...
    <div style="height: 65vh" class="border-2 border-dashed surface-border flex flex-wrap gap-3 overflow-y-scroll p-2">
      <template v-if="viewMode === 'grid'">
        ...
        <Card v-for="session in sessions" :key="session.id"
          ...
          <template #subtitle>
            <div class="p-0 m-0">
              <span class="font-medium text-600 font-bold">{{ session.firstname }} {{ session.lastname }}</span>
  ...
  <Dialog v-model:visible="newSessionVisible" modal header="New Session" :style="{ width: '28rem' }" :pt="{
            root: 'border-none',
            mask: {
