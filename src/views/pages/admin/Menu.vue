...
const addMenuItem = async () => {
  try {
    newMenuItem.value.category_id = selectedCategory.value ? selectedCategory.value.id : null;
    await MenuService.addMenuItem(newMenuItem.value);
    toast.add({ severity: 'success', summary: 'Success', detail: 'Menu item added successfully', life: '3000' });
...
const addCategory = async () => {
  try {
    ...
    toast.add({ severity: 'success', summary: 'Success', detail: 'catef added successfully', life: '3000' });
    // Fetch updated menu items   
...
const archiveSelected = async () => {
  try {
    ...
    toast.add({ severity: 'success', summary: 'Success', detail: 'Menu items archived successfully', life: '3000' });
    // Refresh the menu items after archiving
    menuItems.value = await MenuService.getAllMenuItems();
  } catch (error) {
    console.error('Error archiving menu items:', error);
    toast.add({ severity: 'warn', summary: 'Nothing to archive', detail: 'Select an item to archive', life: '3000' });
  }
};
...
const unarchiveSelected = async () => {
  try {
    ...
    toast.add({ severity: 'success', summary: 'Success', detail: 'Menu items unarchived successfully', life: '3000' });
    menuItems.value = await MenuService.getAllMenuItems();
  } catch (error) {
    console.error('Error archiving menu items:', error);
    toast.add({ severity: 'warn', summary: 'Nothing to unarchive', detail: 'Select an item to unarchive', life: '3000' });
  }
};
...
<template>
        ...
        <DataTable :value="filteredMenuItems" dataKey="id" class="w-full" tableStyle="min-width: 60rem"
          ...
          <Column header="Image">
            <template #body="slotProps">
              <img src="@/assets/images/sisig.jpg" :alt="slotProps.data.image" class="w-6rem border-round" />
            </template>
          </Column>
  ...
  <Dialog v-model:visible="newMenuVisible" modal header="New Menu Item" :style="{ width: '40rem' }">
    <div class="flex-row align-items-center gap-3 mb-3 mt-3 w-full">
      <FloatLabel>
